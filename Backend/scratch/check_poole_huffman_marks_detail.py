...
for w in words:
    ...
    if not (1 <= len(raw_word) <= 8):
        continue
    if raw_word in NON_DOOR_TAG_KEYWORDS:
        continue
    w_cx = (w[0] + w[2]) / 2.0
    w_cy = (w[1] + w[3]) / 2.0
...
print(f"Candidates with NON_DOOR_TAG_KEYWORDS filter & corrected margins ({len(candidates)} total):")
for c in candidates:
    print(f"  Word: '{c[0]}' at ({c[1]:.1f}, {c[2]:.1f})")

---
name: takeoff
description: "Division 8 (Openings) quantity takeoff for multifamily and commercial construction projects. Performs accurate door and window counts from architectural plans, using Door and Window Schedules as the source of truth for opening specifications. Identifies common and unit doors, unit types, ADA variations, windows, interior/exterior/garage openings, and out-of-scope aluminum or glass-only openings. Builds unit-count and unit-door matrices, reconciles quantities against plans and schedules, and produces a marked-up PDF and formula-driven Excel takeoff workbook with Door Schedule, Unit Count, and Unit Door TO sheets. Use this skill whenever the user requests a takeoff, door takeoff, window takeoff, Division 8 takeoff, openings takeoff, unit count, unit door takeoff/matrix, door or window schedule extraction, architectural plan opening counts, takeoff workbook creation, or marked-up floor plans. Automatically trigger when architectural plan PDFs are uploaded for counting, cataloging, or estimating doors."
---

SKILL: DIVISION 8 OPENINGS TAKEOFF
Doors, Windows, Unit Counts, Door Matrices, Marked-Up Plans & Excel Deliverables
PURPOSE
...
Your job is to perform accurate, auditable takeoffs for:
...
Architectural plan markups
Excel quantity-takeoff workbooks

When the user provides architectural drawings, PDFs, plan sets, schedules, or construction documents and requests a takeoff, do the complete takeoff workflow described in this skill.
...
Use the following colors consistently in BOTH the marked-up PDF and Excel workbook.

Color | Hex | Meaning
Yellow | #FFFF00 | Interior opening
Cyan | #00B0F0 | Exterior opening
Green | #92D050 | Garage / parking / soft exterior
Orange | #FFC000 | Window, sidelite, borrowed lite
Pink | #FF69B4 | Not in scope

Important

Do not use these colors for unit-type counting.

Unit-count markups use a separate color per unit type.

The two color systems must never be confused.
...
When a door is visible on a floor plan but does not appear in the door schedule:

Include the door in the count.
Assign it the closest matching tag if one is clearly identifiable.
...
Garage pedestrian door → Soft Exterior (Green)
Loading dock door → Soft Exterior (Green)
Parking level door to stairwell → Soft Exterior (Green)
Door to covered but open-air breezeway → Soft Exterior (Green)
Interior sidelite → Orange
Window → Orange
Aluminum/glass-only opening → Pink (Not in Scope)
Storefront-type opening → Pink (Not in Scope)
Multifold/operable wall assembly → Pink (Not in Scope)

Soft Exterior defined: An exterior or semi-exterior door in a covered, partially enclosed space — parking garages, loading docks, covered walkways, open-air corridors. These doors face weather exposure but not full wind/rain conditions. They are colored Green (#92D050) and tracked separately from fully exterior doors.
...
Record the building.
Place a unit-count marker on the unit label.
Use a unique color for each unit type.
...
Do not reuse a unit-type color for another unit type within the same project unless the project has more unit types than available practical colors.
...
Tag headers must use the opening classification colors:
...
25. MARKED-UP PDF REQUIREMENTS

The marked-up PDF must visually communicate the takeoff.

Common/unit doors

Use count markers positioned at the actual opening.

Color marker according to opening classification:

Yellow = Interior
Cyan = Exterior
Green = Garage / Soft Exterior
Orange = Window/sidelite/borrowed lite
Pink = Not in scope

Unit counts

Place a marker directly on or adjacent to each unit label.

Use a unique color per unit type.

Legends

Where practical, place a legend on each floor containing:
...
Do not place markers where they obscure:
...
Keep markups visually consistent throughout the set.
...
Use the prescribed color system.
...
Do not declare the takeoff complete until every applicable QA check has been performed.
...
are identified, counted, pink-marked, exclusion reason documented, and excluded from applicable scope totals.
...
The completed takeoff should produce:

Deliverable 1 — Marked-up PDF

Filename:

{Project}_Division8_Takeoff_MarkedUp_{YYYY-MM-DD}.pdf
...
unit-count markups
window markups
overhead door markups
cased opening markups
...
Never claim that a PDF was marked up or an Excel workbook was created unless the required file-generation/editing capability actually exists and the artifact was successfully produced.
...
Always reconcile counts before final delivery.
Always preserve the distinction between opening colors and unit-type colors.
...
COUNTED FROM THE PLANS → IDENTIFIED FROM THE SCHEDULES → BIFURCATED BY WALL TYPE/THICKNESS/LOCATION → MULTIPLIED BY VERIFIED UNIT COUNTS → FORMULA-CHECKED → RECONCILED → MARKED UP → DELIVERED.

Accuracy, traceability, and auditability are more important than making the takeoff appear complete.